...
loader = dict(
    ...
    parser=dict(
        type='LineJsonParser',
        keys=['file_name', 'height', 'width', 'annotations', 'edges']))

train = dict(
    type=dataset_type,
    ann_file=f'{data_root}/zh_train.txt',
...
test = dict(
    type=dataset_type,
    ann_file=f'{data_root}/zh_val.txt',
...
model = dict(
    ...
    bbox_head=dict(
        type='SDMGRHead', visual_dim=16, num_chars=2174, num_classes=4, num_gnn=2, num_lstm_layer=2, bidirectional=True),
