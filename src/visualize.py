...
def imshow_edge_node(img,
    ...
    if draw_edge:
        edge_pred_score = result['edges'].detach().cpu()[:, 1]
        edge_pred_score_mtx = edge_pred_score.view(
            (len(boxes), len(boxes)))
        for i, edge_pred_score_row in enumerate(edge_pred_score_mtx):
            start_xmin, start_ymin, start_xmax, start_ymax = int(boxes[
                i][0]), int(boxes[i][1]), int(boxes[i][2]), int(boxes[i][3])
            for j, score in enumerate(edge_pred_score_row):
                if score >= 0.5:
                    end_xmin, end_ymin, end_xmax, end_ymax = int(boxes[
                        j][0]), int(boxes[j][1]), int(boxes[j][2]), int(boxes[j][3])
                    if start_xmax < end_xmin:
                        start_x = start_xmax
                        start_y = (start_ymin + start_ymax) // 2
                        end_x = end_xmin
                        end_y = (end_ymin + end_ymax) // 2
                    elif start_xmin > end_xmax:
                        start_x = start_xmin
                        start_y = (start_ymin + start_ymax) // 2
                        end_x = end_xmax
                        end_y = (end_ymin + end_ymax) // 2
                    elif start_ymax < end_ymin:
                        start_x = (start_xmin + start_xmax) // 2
                        start_y = start_ymax
                        end_x = (end_xmin + end_xmax) // 2
                        end_y = end_ymin
                    elif start_ymin > end_ymax:
                        start_x = (start_xmin + start_xmax) // 2
                        start_y = start_ymin
                        end_x = (end_xmin + end_xmax) // 2
                        end_y = end_ymax
